...
class Sam3Processor:
    ...
    def set_image(self, image,pos_embed, state=None):
        ...
        state["backbone_out"] = self.model.backbone.forward_image(image,pos_embed)
    ...
    def reset_all_prompts(self, state: Dict):
        ...
        keys_to_del = ["geometric_prompt", "boxes_embeds",'conf_threshold',"trans_boxes", "prompts_embeds","prompts_masks", "out_probs_all", "queries", "is_segment", "boxes", "masks", "masks_logits", "scores"]
    ...
    def forward_grounding(self, state: Dict):
        outputs = self.model.forward_grounding(
            boxes_embeds = state['boxes_embeds'],
            backbone_out=state["backbone_out"],
            find_input=self.find_stage,
            geometric_prompt=state["geometric_prompt"],
            find_target=None,
            is_segment = state['is_segment'] if "is_segment" in state else False
        )
        ...
        #####
        state['prompts_embeds'], state['prompts_masks']= (outputs['prev_encoder_out']['encoder_out']["prompt_before_enc"],outputs['prev_encoder_out']['encoder_out']["prompt_mask"])
        state['out_probs_all'] = out_probs
        state["queries"] = outputs["queries"]
        #####
        if 'conf_threshold' in state:
            self.confidence_threshold = state['conf_threshold']
        keep = out_probs > self.confidence_threshold
        out_probs = out_probs[keep]
        ...
        if state['is_segment'] if "is_segment" in state else False:
            out_masks = outputs["pred_masks"]
            out_masks = out_masks[keep]
            out_masks = interpolate(
                out_masks.unsqueeze(1),
