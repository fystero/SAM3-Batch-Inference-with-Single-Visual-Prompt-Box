...
class SequenceGeometryEncoder(nn.Module):
    ...
    def encode_boxes(self, boxes, img_feats, boxes_labels=None):
        boxes_embed = None
        n_boxes, bs = boxes.shape[:2]
        ...
        if boxes_labels is not None:
            type_embed = self.label_embed(boxes_labels.long())
            boxes_embed = type_embed + boxes_embed
        return boxes_embed
    ...
    def forward(self,boxes_embeds, geo_prompt: Prompt, img_feats, img_sizes, img_pos_embeds=None):
        ...
        if not self.encode_boxes_as_points:
            if boxes_embeds is None:
                ...
                boxes_embeds = boxes_embeds
                boxes_mask = torch.zeros(img_feats.shape[0],boxes_embeds.shape[0],dtype=torch.bool,device=boxes_embeds.device)
